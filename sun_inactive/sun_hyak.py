...
from friedrich.mtwilson import get_simulated_solar_transit
...
if os.path.exists('/Users/bmmorris/data/hat11/'):
    ...
    output_dir = os.path.abspath('/gscratch/stf/bmmorris/friedrich/sun_inactive')
    ...
    mwo_dir = '/local/tmp/Mt_Wilson_Tilt'
else:
    raise ValueError('No input files found.')

hat11_oot_params = hat11_params_morris()
# Offset by phase+=0.25
...
lc.subtract_add_divide_without_outliers(params=hat11_oot_params,
                                        quarterly_max=quarterly_maxes[lc.quarters[0]],
                                        plots=False)

## Inject sunspots:
first_kepler_transit = transits[0].times.jd.mean()

# Offset start time by 1/2 the solar cycle duration from the most active time in the cycle
start_time = (Time('1957-01-01') + 6.5 * u.year +
...
transit_model = generate_lc_depth(lc.times_jd, hat11_oot_params.rp**2, hat11_oot_params)
...
if best_fit_spot_params is not None:
    ...
    sampler = run_emcee_seeded(lc, hat11_oot_params, best_fit_spot_params,
